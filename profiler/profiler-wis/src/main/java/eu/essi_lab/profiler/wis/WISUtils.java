...
public class WISUtils {
	...
	public static String getUrl(WebRequest webRequest) {
		...
		try {
			...
			url = uri.getBaseUri().toString() + "/" + viewPart + WISProfiler.WIS_SERVICE_INFO.getServicePath();
		} catch (Exception e) {
		}
		return url;
	}
	...
	public static JSONObject mapFeature(GSResource resource) throws GSException {
		...
		Iterator<Keywords> keywords = resource.getHarmonizedMetadata().getCoreMetadata().getDataIdentification()
				.getKeywords();
		while (keywords.hasNext()) {
			...
			for (CharacterStringPropertyType keyType : keys) {
				...
				if (keyValue instanceof AnchorType) {
					...
					GSLoggerFactory.getLogger(WISUtils.class).error("Unexpcted string type: {}",
		...
		WISUtils.addTheme(feature, "http://codes.wmo.int/wis/topic-hierarchy/earth-system-discipline",
				new Concept("hydrology", "Hydrology",
		...
		List<ResponsibleParty> parties = resource.getHarmonizedMetadata().getCoreMetadata().getMIMetadata()
				.getDataIdentification().getPointOfContactParty();
	...
	public static ResultSet<GSResource> getMetadataItems(String metadataIdentifier, Optional<String> optionalView)
			throws GSException {
	...
	public static void enrichFeaturesWithLinks(JSONArray features, WebRequest webRequest) {
		for (int i = 0; i < features.length(); i++) {
			...
			WISUtils.addLink(links, "application/json", "canonical", id,
					url + "/collections/discovery-metadata/items/" + id);
