	...
	public Response handle(WebRequest webRequest) throws GSException {
		...
		if (!optionalView.isPresent()) {
			ByteArrayInputStream stream = new ByteArrayInputStream(
					"A view identifier is needed".getBytes(StandardCharsets.UTF_8));
...
//		WebRequestTransformer.setView(viewId, uri, discoveryMessage);
				...
				ByteArrayInputStream stream = new ByteArrayInputStream(
						"<html><body>No view provided!</body></html>".getBytes());
			...
			Bond bond = BondFactory.createResourcePropertyBond(BondOperator.EQUAL, ResourceProperty.SOURCE_ID,
			...
			for (int i = 0; i < resources.size(); i++) {
				...
				try {
					...
					variableName = resource.getHarmonizedMetadata().getCoreMetadata().getMIMetadata()
							.getCoverageDescription().getAttributeTitle();
					variableDescription = resource.getHarmonizedMetadata().getCoreMetadata().getMIMetadata()
							.getCoverageDescription().getAttributeDescription();
					...
					variableCode = resource.getHarmonizedMetadata().getCoreMetadata().getMIMetadata()
							.getCoverageDescription().getAttributeIdentifier();
					...
					if (resource.getExtensionHandler().getTimeResolutionDuration8601().isPresent()) {
						intendedObservationSpacing = resource.getExtensionHandler().getTimeResolutionDuration8601()
								.get();
					}
				...
				if (variableURI != null) {
					if (variableName!=null && scr.hasConcept(variableURI)) {
						scr.addTerm(s.getUniqueIdentifier(), variableURI, variableName);
					}
				}
				...
				String csv = uniqueVariableCode + "\t" + protocol + "\t" + variableCode + "\t" + variableName + "\t"
						+ variableDescription + "\t\n";
...
//			}
			...
			content += "<tr><td colspan='15'><br/>"//
					...
					+ "End:" + stats.getEnd() + "<br/>"//
					+ "BBOX(w,s,e,n): " + stats.getWest() + "," + stats.getSouth() + "," + stats.getEast() + ","
					+ stats.getNorth() + "<br/>" //
