 ...
 * @author boldrini
 */
public class GetSiteInfoObjectValidator implements WebRequestValidator {

    public GetSiteInfoObjectValidator() {
    }

    @Override
    public ValidationMessage validate(WebRequest request) throws GSException {	
	try {
	    GetSiteInfoObjectRequest sir = new GetSiteInfoObjectRequest(request);
	    String siteCode = sir.getParameterValue(Parameter.SITE_CODE);
	    if (siteCode == null || siteCode.isEmpty()) {
		ValidationMessage message = new ValidationMessage();
		message.setResult(ValidationResult.VALIDATION_FAILED);
		message.setError("Missing parameter: site");
		message.setLocator("site");
		return message;
	    }
	    ValidationMessage message = new ValidationMessage();
