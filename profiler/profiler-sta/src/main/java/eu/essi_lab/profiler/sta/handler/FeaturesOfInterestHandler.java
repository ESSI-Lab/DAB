...
/**
 * Handler for OGC STA FeaturesOfInterest entity set.
 */
public class FeaturesOfInterestHandler extends StreamingRequestHandler {

    private DatabaseExecutor executor;

    public FeaturesOfInterestHandler() {
	try {
	...
	return output -> {
	    try {
		writeFeaturesOfInterestResponse(output, webRequest);
	    } catch (Exception e) {
		throw new WebApplicationException("Error handling FeaturesOfInterest request", e);
	    }
	};
    }
	...
	return new FeaturesOfInterestTransformer();
    }

    private void writeFeaturesOfInterestResponse(OutputStream output, WebRequest webRequest) throws Exception {
	if (executor == null) {
	...
	List<JSONObject> foi = new ArrayList<>();
