...
public class WMSGetMapHandler extends StreamingRequestHandler {
	...
	try {
		...
		public void write(OutputStream output) throws IOException, WebApplicationException {
			...
			try {
				...
				StationsStatistics stats = dataCacheConnector.getStationStatisticsWithProperties(bbox, false, props);
				...
				if (count > 8000) {
				    ...
				    ig2.drawString("zoom", pixsMinX + (pixsMaxX - pixsMinX) / 2 - 20, pixsMinY + (pixsMaxY - pixsMinY) / 2);
					...
					stations = dataCacheConnector.getStationsWithProperties(bbox, 0, 10000, false, props);
				    } else {
	...
	synchronized (cachedLayers) {
		...
		if (view.isPresent()) {
		    String v = view.get();
		    ClusterType cluster = ClusterType.get();
		    if (v.equals("i-change") && cluster != null && cluster.equals(ClusterType.PRODUCTION)) {
			return null;
