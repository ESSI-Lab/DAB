...
public class DatahubJsonMapper extends DiscoveryResultSetMapper<String> {
	...
	try {
	    ...
	    GSLoggerFactory.getLogger(DatahubJsonMapper.class).warn("Error mapping GSResource to DataHub JSON: {}", e.getMessage(), e);
	...
	try {
		...
		for (DQResultPropertyType resultProperty : results) {
			...
			if (recordProperty != null && recordProperty.isSetRecord()) {
			    ...
			    if (record instanceof RecordTypeType) {
				rawValue = ((RecordTypeType) record).getValue();
			    } else if (record != null) {
