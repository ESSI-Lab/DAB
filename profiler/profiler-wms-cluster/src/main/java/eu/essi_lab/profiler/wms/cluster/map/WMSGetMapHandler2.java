...
public class WMSGetMapHandler2 extends WMSGetMapHandler {
				...
				public void write(OutputStream output) throws IOException, WebApplicationException {
					...
					try {
						...
						DatabaseExecutor executor = DatabaseProviderFactory
								.getExecutor(ConfigurationWrapper.getDatabaseURI());
						...
						for (int i = 0; i < divisions; i++) {
							for (int j = 0; j < divisions; j++) {
								...
								System.out.println("Original bbox: minx " + tmpBboxMinX + " miny " + tmpBboxMinY
										+ " maxx " + tmpBboxMaxX + " maxy " + tmpBboxMaxY);
								if (outputCRS.contains("3857")) {
						...
						for (WMSClusterResponse response : responseList) {
							...
							if (response.getMap().isPresent()) {
								...
								int pieDiameter = (int) (maxDiameter * ((double) totalCount)
										/ ((double) maximumClusterSize));
								...
								for (int i = 0; i < percentages.size(); i++) {
									...
									ig2.fillArc(pieMinX, pieMinY, pieDiameter, pieDiameter,
								...
								ig2.fillRoundRect(centerLabelX - labelWidth / 2 - padding,
										centerLabelY - labelHeight / 2, labelWidth + 2 * padding, labelHeight, 5, 5);

								// Draw the black border around the white background
								ig2.setColor(addTransparency(Color.BLACK));
								ig2.drawRoundRect(centerLabelX - labelWidth / 2 - padding,
										centerLabelY - labelHeight / 2, labelWidth + 2 * padding, labelHeight, 5, 5);

								// Draw the label text
								ig2.setColor(addTransparency(Color.BLACK));
								ig2.drawString(centerLabel, centerLabelX - labelWidth / 2,
										(int) (centerLabelY + labelHeight / 2.8));
								...
								if (debug) {
									...
									if (offsetX != 0 || offsetY != 0) {
										...
										ig2.drawLine(centroidX - crossSize, centroidY - crossSize,
												centroidX + crossSize, centroidY + crossSize);
										ig2.drawLine(centroidX - crossSize, centroidY + crossSize,
												centroidX + crossSize, centroidY - crossSize);
										ig2.setColor(addTransparency(Color.gray));
								...
								for (StationRecord station : stations) {
									...
									ig2.fillOval(stationMinX, stationMinY, stationDiameterInPixels,
											stationDiameterInPixels);
									Color g = Color.black;
									if (offsetX != 0 || offsetY != 0) {
										g = Color.gray;
									}
									...
									ig2.drawOval(stationMinX, stationMinY, stationDiameterInPixels,
											stationDiameterInPixels);
	...
	private Bond getConstraints(WebRequest webRequest) {
		...
		addSimpleValueBonds(parser, "attributeTitle", andBond, MetadataElement.ATTRIBUTE_TITLE);
...
//	}
	...
	private void addSimpleValueBonds(KeyValueParser parser, String propertyName, LogicalBond andBond,
