...
GWIS._nwisRootUrl = "gwis"; // https enabled 12/14/2016
            ...
            "axes": {
                "x": {
                    ...
                    "valueFormatter" : function(ms) { // format function for
														...
														// legend
                        var d = new Date(ms);
                        var D = [ d.getFullYear(), d.getMonth()+1, d.getDate() ].join("-"); // yyyy-mm-dd
...
GWIS._refreshPlot = function (plot) {
    ...
    $.each( plot.opts.series, function(idx,ser) {
        ...
        $.ajax({
            "type"     : "GET",
            "url"      : url + "&sites="+ser.site + "&parmCd="+ser.pcode + (ser.dv_stat!==undefined ? "&statCd="+ser.dv_stat : ""), // add
																																	// site,
																																	// pcode,
																																	// and
																																	// dv_stat
...
GWIS._plotError = function( opts, msg ) {
    ...
    if (GWIS._isLoadedJq && opts!==undefined && opts.div_id!==undefined) {
        $("#"+opts.div_id)
            ...
            .html( '<div class="gwis-center-me">No recent data available. Please select a long time period for this time series and update the plot.</div>' );
        GWIS._center_mes();
    }
