...
public class TermFrequencyMap extends DOMSerializer {
	 ...
	 * 
	 */
	OBSERVED_PROPERTY_URI(MetadataElement.OBSERVED_PROPERTY_URI_EL_NAME),
	/**
	 * 
	 */
	...
	List<TermFrequencyItem> observedPropertyUri = type.getObservedPropertyURI();
	...
	if (!observedPropertyUri.isEmpty()) {
	    targets.add(TermFrequencyTarget.OBSERVED_PROPERTY_URI.getName());
	}
    }
	...
	for (String target : targets) {
	    ...
	    // 1: creates the union list. can contains items with the same term, they must
	    // be merged
