 ...
 * @author boldrini
 */
public class MWRIConnector extends HarvestedQueryConnector<MWRIConnectorSetting> {

    /**
     * 
     */
    public static final String TYPE = "MWRIConnector";
    /**
     * 
     */
    private static final String MWRI_CONNECTOR_SOURCE_NOT_FOUND_ERROR = "MWRI_CONNECTOR_SOURCE_NOT_FOUND_ERROR";

    @Override
    public boolean supports(GSSource source) {
	String url = source.getEndpoint();
	return url.contains("mwri.gov.eg");
    }

    private static String NS = "mwri.gov.eg";
	...
	MWRIClient client = new MWRIClient(getSourceURL());
	...
	if (!source.isPresent()) {
	    ...
	    throw GSException.createException(//
		    ...
		    MWRI_CONNECTOR_SOURCE_NOT_FOUND_ERROR);
	}
	...
	try {
		...
		for (MWRIParameter variable : MWRIParameter.values()) {
		    ...
		    String stationId = station.getName();
		    String parameterId = variable.toString();
		    ...
		    String attributeTitle = variable.toString();
		    ...
		    temporalExtent.setBeginPosition(ISO8601DateTimeUtils.getISO8601DateTime(station.getDate()));
		    temporalExtent.setEndPosition(ISO8601DateTimeUtils.getISO8601DateTime(station.getDate()));
		    ...
		    // datasetContact.setOrganisationName("");
		    // datasetContact.setRoleCode("publisher");
		    //
		    // coreMetadata.getMIMetadata().getDataIdentification().addPointOfContact(datasetContact);

		    coreMetadata.getMIMetadata().getDataIdentification().setCitationTitle(station.getName() + " - " + variable.toString());
		    ...
		    coreMetadata.getMIMetadata().getDataIdentification()
			    .setAbstract("Acquisition made at station: " + station.getName() + " Parameter: " + variable.toString());
		    ...
		    coreMetadata.addDistributionOnlineResource(identifier, getSourceURL(), MWRIDownloader.MWRI_NS, "download");
		    ...
		    dataset.getExtensionHandler().setCountry(Country.EGYPT.getShortName());
    ...
    @Override
    protected MWRIConnectorSetting initSetting() {

	return new MWRIConnectorSetting();
    }
}
