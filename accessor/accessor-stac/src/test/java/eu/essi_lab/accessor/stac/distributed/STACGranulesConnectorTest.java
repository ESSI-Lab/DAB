 ...
 * @author ilsanto
 */
public class STACGranulesConnectorTest {
	...
	STACDEAGranulesConnector connector = Mockito.spy(new STACDEAGranulesConnector());
	...
	STACGranulesBondHandler handler = Mockito.mock(STACGranulesBondHandler.class);

	String baseurl = "https://explorer.digitalearth.africa/stac/search?";
	...
	Assert.assertEquals(baseurl + handledQuery, req);
	...
	InputStream stream = STACGranulesConnectorTest.class.getClassLoader().getResourceAsStream("deaSTACGranulesOriginal.json");
	...
	try {
	    ...
	    Assert.assertEquals(converted.get(0).getSchemeURI(), STACGranulesResultMapper.STAC_GRANULES_SCHEME_URI.toString());

	    JSONObject j = new JSONObject(converted.get(0).getMetadata());

	    Assert.assertEquals("002acef8-727a-5e3f-bce7-50a4a7d5ec6f", j.getString("id"));
	...
	int count = 3;
	int start = 1;
	STACGranulesBondHandler handler = connector.parse(message, start, count, "");
