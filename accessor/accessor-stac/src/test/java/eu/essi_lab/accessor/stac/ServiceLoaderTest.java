...
import eu.essi_lab.accessor.stac.distributed.STACDEAGranulesConnector;
...
import eu.essi_lab.accessor.stac.harvested.STACConnectorWrapper;
...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(STACMixedDistributedAccessor.class, accessors.get(0).getClass());

	    Assert.assertEquals(STACMixedDistributedAccessor.TYPE, accessors.get(0).getType());
...

	{
	    ...
	    Assert.assertEquals(STACMixedHarvestedAccessor.class, accessors.get(0).getClass());

	    Assert.assertEquals(STACMixedHarvestedAccessor.TYPE, accessors.get(0).getType());
	}
	...
	gsSource.setEndpoint("https://explorer.digitalearth.africa/stac/collections");
	...
	Assert.assertEquals(STACMixedDistributedAccessor.class, accessor.getClass());
    }
	...
	Assert.assertEquals(STACMixedHarvestedAccessor.class, accessor.getClass());
    }
	...
	Assert.assertEquals(STACDEAGranulesConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertEquals(STACConnectorWrapper.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(STACGranulesResultMapper.STAC_GRANULES_SCHEME_URI)).findFirst()
		.isPresent());

	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(STACCollectionMapper.STAC_SCHEME_URI)).findFirst().isPresent());
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(STACDEAGranulesConnector.class.getName())).//
		findFirst().//
		isPresent());

	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(STACMixedDistributedAccessor.class.getName())).//
		findFirst().//
		isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(STACConnectorWrapper.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(STACMixedHarvestedAccessor.class.getName())).//
		findFirst().//
		isPresent());
