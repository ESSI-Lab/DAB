 ...
 * @author Fabrizio
 */
public class SOSConnector extends WrappedConnector {

    protected static final String SOS_CONNECTOR_DOWNLOAD_ERROR = "SOS_CONNECTOR_DOWNLOAD_ERROR";
    private static final String SOC_CONNECTOR_LIST_RECORDS_ERROR = "SOC_CONNECTOR_LIST_RECORDS_ERROR";
    ...
    private HashMap<String, GetFeatureOfInterestResponseType> featuresCache = new HashMap<>();
    ...
    /**
     * Returns a map procedure description format to procedure description, describing a procedure with all the
     * available formats
     * 
     * @param procedure
    ...
    private HashMap<String, DescribeSensorResponseType> retrieveProcedureDescriptions(String procedure,
	...
	try {
		...
		GSLoggerFactory.getLogger(getClass()).info(
			"Serving offering " + offeringIndex + "/" + offeringSize + " (feature " + featureIndex + "/" + featuresSize + ")");
		...
		if (observableProperties.isEmpty()) {
		    // in case observable properties list is empty, those are inherited from contents
		    observableProperties = new HashSet<String>(capabilities.getContents().getContents().getObservableProperty());
		}
		...
		for (String observableProperty : observableProperties) {
			...
			GetDataAvailabilityResponseType availabilityResponse = retrieveDataAvailability(procedure, featureIdentifier,
				observableProperty);
			...
			for (DataAvailabilityMemberType availabilityMember : availabilityMembers) {
				...
				if (abstractTime != null) {
					...
					if (timeObject instanceof TimePeriodType) {
					    ...
					    temporals.add(new SimpleEntry<String, String>(begin, end));
			...
			metadata.setProperty(SOSProperty.DOWNLOAD_PROTOCOL, getDownloadProtocol());
					...
					if (sensorDescriptionObject != null) {
						...
						if (any != null) {
							...
							if (format != null && format.equals("http://www.opengis.net/sensorml/2.0")) {
							    ...
							    Node[] keywordNodes = reader
								    .evaluateNodes("/*:PhysicalSystem/*:keywords/*:KeywordList/*:keyword");
							    ...
							    metadata.setProperty(SOSProperty.SENSOR_ContactManufacturerOrganization,
								    reader.evaluateString(
							    ...
							    metadata.setProperty(SOSProperty.SENSOR_ContactOwnerPhone,
								    reader.evaluateString(
									    "/*:SensorML/*:member/*/*:contact[@*:arcrole='urn:ogc:def:classifiers:OGC:contactType:owner']/*:ResponsibleParty/*:contactInfo/*:phone/*:voice"));
							    metadata.setProperty(SOSProperty.SENSOR_ContactOwnerAddressDeliveryPoint,
								    reader.evaluateString(
							    ...
							    metadata.setProperty(SOSProperty.SENSOR_ContactOwnerAddressCity,
								    reader.evaluateString(
									    "/*:SensorML/*:member/*/*:contact[@*:arcrole='urn:ogc:def:classifiers:OGC:contactType:owner']/*:ResponsibleParty/*:contactInfo/*:address/*:city"));
							    metadata.setProperty(SOSProperty.SENSOR_ContactOwnerAddressCountry,
								    reader.evaluateString(
							    ...
							    metadata.setProperty(SOSProperty.SENSOR_DocumentationImageFormat,
								    reader.evaluateString(
									    "/*:SensorML/*:member/*/*:documentation[@*:arcrole='urn:ogc:def:object:OGC:1.0:image']/*:Document/*:format"));
							    metadata.setProperty(SOSProperty.SENSOR_DocumentationImageDescription,
								    reader.evaluateString(
