...
public class SOSMapper extends OriginalIdentifierMapper {
	...
	protected GSResource execMapping(OriginalMetadata originalMD, GSSource source) throws GSException {
		...
		if (elevation != null) {
			...
			addVerticalExtent(Double.valueOf(elevation),
		...
		if (properties.getProperty(SOSProperty.SENSOR_Name) != null) {
			coreMetadata.getMIMetadata().getDataIdentification()
					.addKeyword(properties.getProperty(SOSProperty.SENSOR_Name));
		}

		if (properties.getProperty(SOSProperty.SENSOR_Description) != null) {

			coreMetadata.getMIMetadata().getDataIdentification()
					.addKeyword(properties.getProperty(SOSProperty.SENSOR_Description));
		}
		...
		if (serviceProviderOrganization != null) {
			...
			if (properties.getProperty(SOSProperty.SERVICE_PROVIDER_AddressEmailAddress) != null) {
				address.addElectronicMailAddress(
						properties.getProperty(SOSProperty.SERVICE_PROVIDER_AddressEmailAddress));
			}
		...
		if (properties.getProperty(SOSProperty.SENSOR_ContactManufacturerOrganization) != null) {
			sensorInfo += " Sensor manufacturer: "
					+ properties.getProperty(SOSProperty.SENSOR_ContactManufacturerOrganization);
		}
		...
		coreMetadata.setAbstract("Sensor observation data from SOS " + getSOSVersion() + " Observed property: "
				+ parameterName + " Station/platform: " + stationName + sensorInfo);
		...
		if (source.getEndpoint().contains("hnapi.hydronet.com") || source.getEndpoint().contains("hn4s.hydronet.com")
			...
			connector = source.getEndpoint().contains("hnapi.hydronet.com")
					|| source.getEndpoint().contains("hn4s.hydronet.com") ? new SOSTAHMOConnector()
			...
			String url = builder.createDataRequest(properties.getProperty(SOSProperty.PROCEDURE_IDENTIFIER),
					properties.getProperty(SOSProperty.FOI_ID),
					properties.getProperty(SOSProperty.OBSERVED_PROPERTY_ID), beforeDate, delayDate);
