...
public abstract class AbstractSOSConnector extends WrappedConnector {
								...
								if (any != null) {
									if (any instanceof Node) {
										...
										if (format != null && format.equals("http://www.opengis.net/sensorml/2.0")) {
												...
												if (contactNodes != null) {
													for (Node contact : contactNodes) {
														...
														metadata.setProperty(
																SOSProperty.SENSOR_ContactOwnerOrganization,
														...
														metadata.setProperty(
																SOSProperty.SENSOR_ContactOwnerAddressEmail,
																contactReader.evaluateString(
											...
											metadata.setProperty(SOSProperty.SENSOR_ManufacturerName,
	...
	/**
	 * Sub classes might modify the feature response stream e.g. to make it XML
	 * valid
	 * 
	 * @param stream
	 * @throws Exception
	...
	/**
	 * Returns a map procedure description format to procedure description,
	 * describing a procedure with all the available formats
	 * 
	 * @param procedure
	...
	protected HashMap<String, DescribeSensorResponseType> retrieveProcedureDescriptions(String procedure,
		...
		for (String procedureDescriptionFormat : procedureDescriptionFormats) {
			String describeSensorEndpoint = builder.createProcedureDescriptionRequest(procedure,
			...
			try {
				...
				GSLoggerFactory.getLogger(getClass())
						.warn("Sensor description retrieval failed for format: " + procedureDescriptionFormat);
			}
		}
