 ...
 * @author Roberto
 */
public class HISCentralBasilicataMapper extends FileIdentifierMapper {

    private SimpleDateFormat iso8601Format;

    @Override
    public String getSupportedOriginalMetadataSchema() {
	return CommonNameSpaceContext.HISCENTRAL_BASILICATA_NS_URI;
    }

    public HISCentralBasilicataMapper() {
	// 2010-06-01T00:00:00
	...
	originalMetadata.setSchemeURI(CommonNameSpaceContext.HISCENTRAL_BASILICATA_NS_URI);
	...
	// DATASET INFO
	// {
	// "stationName": "Abriola a Sellata Pierfaone",
	// "stationId": 653100,
	// "locality": "Sellata",
	// "city": "Abriola",
	// "prov": "PZ",
	...
	// "measUnit": "mm",
	// "stationName": "Abriola a Sellata Pierfaone",
	// "startDate": "2014-06-13T00:00:00",
	// "elementName": "Accumulated Rainfall",
	// "stationId": 653100
	// }
	...
	if (locality != null && !locality.isEmpty()) {
	    coreMetadata.getMIMetadata().getDataIdentification()
		    .setCitationTitle(stationName + " (" + locality + ")" + " - " + measureName);
	    coreMetadata.getMIMetadata().getDataIdentification().setAbstract(stationName + " (" + locality + ")" + " - " + measureName);
	} else {
	    coreMetadata.getMIMetadata().getDataIdentification().setCitationTitle(stationName + " - " + measureName);
	    coreMetadata.getMIMetadata().getDataIdentification().setAbstract(stationName + " - " + measureName);
	}
	...
	publisherContact.setOrganisationName("CAE - Basilicata");
	...
	coreMetadata.getMIMetadata().getDataIdentification().addKeyword("BASILICATA");
	...
	String linkage = !url.endsWith("/") ? url + "/data/" + timeSeriesId + "?from=" + tempExtenBegin
		+ "&type=Plausible&part=IsoTime&part=Value&part=Quality&part=QualityDescr&timing=Original&elab=None" : url + "data/" + timeSeriesId + "?from=" + tempExtenBegin
	...
	online.setProtocol(CommonNameSpaceContext.HISCENTRAL_BASILICATA_NS_URI);
	...
	if (measureName.contains("Igro ")) {
	    measureName = "Igro";
	}
	if (measureName.contains("Igrometro ")) {
	    measureName = "Igrometro";
	}
	if (measureName.startsWith("T ")) {
	    measureName = "T";
	}
	if (measureName.startsWith("Tensiometro ")) {
	    measureName = "Tensiometro";
	}
