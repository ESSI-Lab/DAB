...
import eu.essi_lab.accessor.seanoe.SEANOEAccessor;
import eu.essi_lab.accessor.seanoe.SEANOEConnector;
...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(SEANOEAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(SEANOEConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(SEANOEAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(SEANOEConnector.class.getName())).//
			findFirst().//
			isPresent());
