 ...
 * @author boldrini
 */
public class UKHydrologyDownloader extends WMLDataDownloader {
	...
	String measureNotation = online.getName();
	UKHydrologyClient client = new UKHydrologyClient(online.getLinkage());
	...
	if (begin == null || end == null) {
	    GSLoggerFactory.getLogger(getClass()).warn("No temporal extent available for measure {}", measureNotation);
	    return null;
	}

	UKHydrologyClient client = new UKHydrologyClient(online.getLinkage());
	List<UKHydrologyEntity> values = client.retrieveReadings(measureNotation, begin, end);

	try {
	    TimeSeriesTemplate tsrt = getTimeSeriesTemplate(getClass().getSimpleName(), ".wml");

	    if (!values.isEmpty()) {
		for (UKHydrologyEntity entity : values) {
		    String phenomenonTime = entity.getObject().optString(UKHydrologyClient.READING_DATE_TIME, null);
		    if (phenomenonTime == null || phenomenonTime.isEmpty()) {
			phenomenonTime = entity.getObject().optString("date", null);
		    }
		    double result = entity.getObject().getDouble(UKHydrologyClient.READING_VALUE);
	...
	return (online.getProtocol() != null && online.getProtocol().equals(NetProtocolWrapper.UK_HYDROLOGY.getCommonURN()));
    }
	...
	return new UKHydrologyConnector().supports(source);
    }
}
