...
<!-- 
    ...
    *
    * schematron rules version 12.1.2
    */
-->
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
    ...
    <sch:pattern>
        ...
        <sch:rule context="gmd:MD_Resolution">            
            ...
            <sch:assert test="$catalog//dc:identifier[substring(.,string-length(.) - $length + 1,$length) = $uomValue]"><sch:value-of select="local-name(.)"/>: the given uomValue (<sch:value-of select="$uomValue"/>) is not included in P06 list</sch:assert>            
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="gmd:westBoundLongitude">            
            <sch:assert test="string-length(normalize-space(substring-after(gco:Decimal, '.'))) &gt; 1">Precision of two decimal places is required for westBoundLongitude</sch:assert>
        </sch:rule>
    </sch:pattern>
    <sch:pattern>
        <sch:title>eastBoundLongitude (345) -> restricted domain</sch:title>
        <sch:rule context="gmd:eastBoundLongitude">            
            <sch:assert test="string-length(normalize-space(substring-after(gco:Decimal, '.'))) &gt; 1">Precision of two decimal places is required for eastBoundLongitude</sch:assert>
        </sch:rule>
    </sch:pattern>
    <sch:pattern>
        <sch:title>southBoundLatitude (346) -> restricted domain</sch:title>
        <sch:rule context="gmd:southBoundLatitude">            
            <sch:assert test="string-length(normalize-space(substring-after(gco:Decimal, '.'))) &gt; 1">Precision of two decimal places is required for southBoundLatitude</sch:assert>
        </sch:rule>
    </sch:pattern>
    <sch:pattern>
        <sch:title>northBoundLatitude (347) -> restricted domain</sch:title>
        <sch:rule context="gmd:northBoundLatitude">            
            <sch:assert test="string-length(normalize-space(substring-after(gco:Decimal, '.'))) &gt; 1">Precision of two decimal places is required for northBoundLatitude</sch:assert>
        </sch:rule>
    </sch:pattern>    
    <sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="gmd:EX_VerticalExtent">
            ...
            <sch:assert test="$catalog//dc:identifier[substring(.,string-length(.) - $length + 1,$length) = $identifierValue]"><sch:value-of select="local-name(.)"/>: the given vertical datum identifier (<sch:value-of select="$identifierValue"/>) is not included in L11 list</sch:assert>
    ...
    <sch:pattern>
        ...
        <sch:rule context="*">            
            <sch:assert test="* or normalize-space(text()) or @gco:nilReason or @xlink:href">Empty <sch:value-of select="local-name(../.)"/> / <sch:value-of select="local-name(.)"/>. Empty objects are not permitted unless for optional elements - in that case a reason must be provided - a xlink pointer is also allowed</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="*[@codeList]">
            ...
            <sch:assert test="$catalog//gmx:codelistItem/*[@gml:id=$codeList or @gml32:id=$codeList]/gmx:codeEntry/*[gml:identifier=$codeListValue or gml32:identifier=$codeListValue]"><sch:value-of select="local-name(.)"/>: the given value (<sch:value-of select="$codeListValue"/>) is not allowed by the codelist catalogue</sch:assert>
        </sch:rule>
    </sch:pattern>
