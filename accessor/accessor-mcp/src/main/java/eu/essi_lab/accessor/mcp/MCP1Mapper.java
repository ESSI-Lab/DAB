...
/**
 * Mapper from MCP 1 data model (a profile of ISO 19115 including MCP extended elements)
 * 
 * @author boldrini
 */
public class MCP1Mapper extends FileIdentifierMapper {

    private static final String MCP1_MAPPER_ERROR = "MCP1_MAPPER_ERROR";
	...
	try {
	    ...
	    writer.rename("//mcp1:MD_Metadata", "gmd:MD_Metadata");
	    writer.rename("//mcp1:MD_DataIdentification", "gmd:MD_DataIdentification");
	    writer.rename("//mcp1:EX_TemporalExtent", "gmd:EX_TemporalExtent");
	    // Here the profile codelists are converted to GMD core profile free text
	    writer.rename("//mcp1:*[ends-with(name(), 'Code')]", "gco:CharacterString");
	...
	try {
	    ...
	    switch (namespace) {
	    case CommonNameSpaceContext.MCP_1_NS_URI:
		return true;
