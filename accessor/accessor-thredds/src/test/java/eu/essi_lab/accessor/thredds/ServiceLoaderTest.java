...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(THREDDSAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(THREDDSConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(THREDDSAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(THREDDSConnector.class.getName())).//
		findFirst().//
		isPresent());
