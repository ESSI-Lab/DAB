...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(RasaqmAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(RasaqmConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(CommonNameSpaceContext.RASAQM_URI)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(RasaqmAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(RasaqmConnector.class.getName())).//
			findFirst().//
			isPresent());
