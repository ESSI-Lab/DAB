...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(WEKEOAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(WEKEOConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(CommonNameSpaceContext.WEKEO_NS_URI)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(WEKEOAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(WEKEOConnector.class.getName())).//
			findFirst().//
			isPresent());
