...
import eu.essi_lab.accessor.acronet.AcronetAccessor;
import eu.essi_lab.accessor.acronet.AcronetConnector;
...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(AcronetAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(AcronetConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(CommonNameSpaceContext.ACRONET)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(AcronetAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(AcronetConnector.class.getName())).//
			findFirst().//
			isPresent());
