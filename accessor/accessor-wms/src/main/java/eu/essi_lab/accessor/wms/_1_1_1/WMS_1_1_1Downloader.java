...
public class WMS_1_1_1Downloader extends WMSDownloader {

    protected int PREVIEW_SIZE = 300;

    private WMS_1_1_1Connector connector;
	...
	this.connector = new WMS_1_1_1Connector();
	this.connector.setSourceURL(linkage);
    }

    public WMS_1_1_1Connector getConnector() {
	return connector;
    }

    protected void setConnector(WMS_1_1_1Connector connector) {
	...
	WMTMSCapabilities fullCapabilities = getConnector().retrieveCapabilities();
	...
	WMS_1_1_1Capabilities capabilities = new WMS_1_1_1Capabilities(fullCapabilities);
	WMS_1_1_1Layer layer = new WMS_1_1_1Layer(capabilities, name);
	...
	try {
		...
		if (gridCRS.equals(CRS.OGC_84())) {
		    // EPSG:4326 is added.. it will be sufficient to invert appropriately the BBOX axis parameters
	...
	// 1.1.1 KVP order always min east, min north, max east, max north

	String ret = "&BBOX="; //

	switch (crs.getAxisOrder()) {
	case NORTH_EAST:
	    ret += secondDimension.getContinueDimension().getLower() + "," + //
	...
	case EAST_NORTH:
	default:
	    ret += firstDimension.getContinueDimension().getLower() + "," + //
