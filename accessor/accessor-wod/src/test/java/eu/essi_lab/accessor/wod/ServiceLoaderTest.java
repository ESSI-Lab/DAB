...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(WODAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(WODConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(WODAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(WODConnector.class.getName())).//
			findFirst().//
			isPresent());
