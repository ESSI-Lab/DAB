...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(HealthSitesAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(HealthSitesConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(HealthSitesMapper.HEALTH_SITES_SCHEME_URI)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(HealthSitesAccessor.class.getName())).//
		findFirst().//
		isPresent());

	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(HealthSitesConnector.class.getName())).//
		findFirst().//
		isPresent());
