...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(ICOSAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(ICOSConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(CommonNameSpaceContext.ICOS_NS_URI)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(ICOSAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(ICOSConnector.class.getName())).//
			findFirst().//
			isPresent());
