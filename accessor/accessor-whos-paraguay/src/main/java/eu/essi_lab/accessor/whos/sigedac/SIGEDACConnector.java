 ...
 * @author boldrini
 */
public class SIGEDACConnector extends HarvestedQueryConnector<SIGEDACConnectorSetting> {

    /**
     * 
     */
    public static final String TYPE = "SIGEDACConnector";
    /**
     * 
     */
    private static final String SIGEDAC_CONNECTOR_SOURCE_NOT_FOUND_ERROR = "SIGEDAC_CONNECTOR_SOURCE_NOT_FOUND_ERROR";

    private List<SIGEDACStation> stationList = new ArrayList<SIGEDACStation>();

    private int partialNumbers;

    public enum RESOLUTION {
	HOURLY("hourly"), DAILY("daily"), NULL("null");
	...
	return url.contains("sigedac.meteorologia.gov.py");
    }

    private static String NS = "sigedac.meteorologia.gov.py";
	...
	if (!source.isPresent()) {
	    ...
	    throw GSException.createException(//
		    ...
		    SIGEDAC_CONNECTOR_SOURCE_NOT_FOUND_ERROR);
	}
	...
	try {
		...
		for (RESOLUTION r : RESOLUTION.values()) {
			...
			String parameterId = variable.getId();
			...
			BigDecimal vertical = station.getElevation();
			if (vertical != null) {
			    try {
			...
			String attributeTitle = normalize(variable.getName());
			...
			coverageDescription.setAttributeDescription(variable.getName());

			coreMetadata.getDataIdentification().addKeyword(variable.getName());
			coreMetadata.getDataIdentification().addKeyword(variable.getShortName());

			String cns = CommonNameSpaceContext.SIGEDAC_PARAGUAY_URI;
			
			if(r.resolution.equals("null")) {
			    stationId = station.getCode();
			    cns = CommonNameSpaceContext.SIGEDAC_RIVER_URI;
			    coreMetadata.getMIMetadata().getDataIdentification().addKeyword("river");
			}
			...
			coreMetadata.getMIMetadata().getDataIdentification()
				.setCitationTitle(station.getName() + " - " + normalize(variable.getName()));

			// coreMetadata.getMIMetadata().getDataIdentification().setCitationAlternateTitle(parameterDescription);

			coreMetadata.getMIMetadata().getDataIdentification()
				.setAbstract("Acquisition made at station: " + station.getName() + " Parameter: " + variable.getName());
			...
			coreMetadata.addDistributionOnlineResource(identifier, getSourceURL(), cns, "download");
	    ...
	    e.printStackTrace();
	    System.out.println("ERROR IN DOWNLOADING DATA FOR STATION :" + stationList.get(start).getName());
	    ret.setResumptionToken(String.valueOf(start + 1));
	    return ret;
	}
    ...
    private void populateStations(SIGEDACClient client) throws Exception {
	if (stationList.isEmpty()) {
	    stationList = client.getStations();
	    GSLoggerFactory.getLogger(getClass()).trace("Number of SIGEDAC stations found: {}", stationList.size());
	}

    }

    @Override
    protected SIGEDACConnectorSetting initSetting() {
