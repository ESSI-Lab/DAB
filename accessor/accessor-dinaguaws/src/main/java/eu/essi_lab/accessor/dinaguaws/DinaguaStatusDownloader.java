 ...
 * @author boldrini
 */
public class DinaguaStatusDownloader extends WMLDataDownloader {
	...
	return online.getLinkage().contains(CommonNameSpaceContext.DINAGUA_STATUS_URI);
    }
	...
	return (online.getProtocol() != null && online.getProtocol().equals(NetProtocolWrapper.DINAGUASTATUSWS.getCommonURN()));
    }
	...
	try {
	    ...
	    String stationCode = mangler.getStationIdentifier();
	    String temporalidad = mangler.getTemporalidadIdentifier();
	    DinaguaClient client = new JSONDinaguaClient(online.getLinkage());
	    DinaguaStation station = client.getStatusStation(stationCode);
...
//	    }
	    ...
	    DinaguaData observations = client.getStatusData(stationCode, temporalidad, begin, end);
