...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(PRISMAAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(PRISMAConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(PRISMAMapper.PRISMA_SCHEME_URI)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(PRISMAAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(PRISMAConnector.class.getName())).//
			findFirst().//
			isPresent());
