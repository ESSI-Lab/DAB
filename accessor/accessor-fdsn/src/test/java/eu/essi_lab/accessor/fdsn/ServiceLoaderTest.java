...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(FDSNAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	gsSource.setEndpoint("http://earthquake.usgs.gov/fdsnws/event/");
	...
	Assert.assertEquals(FDSNAccessor.class, accessor.getClass());
	...
	Assert.assertEquals(FDSNConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertEquals(FDSNMapper.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(FDSNAccessor.class.getName())).//
			findFirst().//
			isPresent());

	Assert.assertTrue(
		StreamUtils.iteratorToStream(loader.iterator()).filter(c -> c.getClass().getName().equals(FDSNConnector.class.getName())).//
			findFirst().//
			isPresent());
