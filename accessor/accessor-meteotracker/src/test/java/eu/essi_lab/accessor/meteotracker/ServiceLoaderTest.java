...
public class ServiceLoaderTest {
...

	{
	    ...
	    Assert.assertEquals(MeteoTrackerAccessor.class, disAccessors.get(0).getClass());
	}
	{
	...
	Assert.assertEquals(MeteoTrackerConnector.class, loader.iterator().next().getClass());
    }
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator()).//
		filter(c -> c.getSupportedOriginalMetadataSchema().equals(CommonNameSpaceContext.METEOTRACKER)).//
		findFirst().//
		isPresent());
	...
	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(MeteoTrackerAccessor.class.getName())).//
		findFirst().//
		isPresent());

	Assert.assertTrue(StreamUtils.iteratorToStream(loader.iterator())
		.filter(c -> c.getClass().getName().equals(MeteoTrackerConnector.class.getName())).//
		findFirst().//
		isPresent());
