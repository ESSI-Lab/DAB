...
public class Workblock {
    ...
    /**
     * Creates a new block with the supplied <code>process</code>, <code>input</code> and <code>output</code>
     *
     * @param process
     * @param input
     * @param output
    ...
    /**
     * Creates a new block with the supplied <code>id</code> and <code>process</code>
     *
     * @param id
     * @param process
     */
     ...
     *
     * @param preference an integer >= {@link #MINIMUM_PREFERENCE} AND <= {@link #MAXIMUM_PREFERENCE}
    ...
    /**
     * Finds all the predecessors of this block (if it is contained in an adjacentes list) having at list one
     * predecessor (this excludes the
     * initial blocks) and inserts them in the supplied <code>list</code>
     *
     * @param list
     */
    ...
    /**
     * Return the sum of the preferences of all the predecessors (directs and non) of this block. In the computation
     * brothers blocks (blocks
     * having the same successor) are considered as a single block having as preference the max preference. This is fine
     * because the
     * algorithm which uses this method compares only blocks at the same level in the blocks graph, so even if a block
     * has more predecessors
     * of another, the path which brings to the root has the same length
     */
