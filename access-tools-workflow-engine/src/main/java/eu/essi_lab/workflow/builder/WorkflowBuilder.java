...
/**
 * Builds a set of available {@link Workflow}s of minimum length from a supplied input {@link DataDescriptor} to a
 * supplied target {@link
 * DataDescriptor}
 *
 * @author Fabrizio
 */
public class WorkflowBuilder {
    ...
    /**
     * Creates an instance of loaded with the blocks of all the available {@link Workblock}s
     */
    ...
    /**
     * Builds the preferred workflow available which start from the supplied <code>inputDescriptor</code> and arrive to
     * the supplied
     * <code>outputDescriptor</code>. The returned {@link Optional} is empty if:
     * <ul>
     * <li>there is no initial {@link ProcessorCapabilities} which suitable for the supplied
     * <code>inputDescriptor</code></li>
     * <li>there is no terminal {@link ProcessorCapabilities} which suitable for the supplied
     * <code>outputDescriptor</code></li>
     * <li>there is no path which starting from the initial {@link ProcessorCapabilities}/s arrives to the terminal
     * {@link ProcessorCapabilities}/s</li>
     * </ul>
     * The preference is given according to the preferences of the all the workflows built by the
     * {@link #create(InputDescriptor,
     * OutputDescriptor)} method<br>
     *
    ...
    /**
     * Builds the available workflows which start from the supplied <code>inputDescriptor</code> and arrive to the
     * supplied
     * <code>outputDescriptor</code>. The returned list is empty if:
     * <ul>
     * <li>there is no initial {@link ProcessorCapabilities} suitable for the supplied
     * <code>inputDescriptor</code></li>
     * <li>there is no terminal {@link ProcessorCapabilities} suitable for the supplied
     * <code>outputDescriptor</code></li>
     * <li>there is no path which starting from the initial {@link ProcessorCapabilities}/s arrives to the terminal
     * {@link ProcessorCapabilities}/s</li>
     * <li>there are paths which starting from the initial {@link ProcessorCapabilities}/s and arrive to the terminal
     * {@link ProcessorCapabilities}/s, but the length is greater than {@link #getMaxWorkflowLength()}</li>
     * </ul>
     * <h3>Building stages</h3>
     * The workflows building operates on 5 stages:<br>
     * <br>
     * <ol>
     * <li>Initial and terminal block detection</li>
     * In this stage the blocks suitable with the supplied supplied <code>inputDescriptor</code> and
     * <code>outputDescriptor</code> are marked as <i>initial</i> and/or <i>terminal</i>. Each initial block represents
     * the root of the correspondent tree, and each terminal block a leaf (see 3.)
     * <li>Graph creation</li>
     * In this stage a cartesian power of second degree (excluding the identical couples) is applied on the builder
     * blocks. Distinct couples
     * of blocks are linked according to their input and output
     * <li>Trees traversal</li>
     * The graph is split by selecting a sub-graph for each initial block, thus creating trees with a root (the initial
     * block) and one or
     * more leafs (terminal blocks). Each tree is traversed using the "breadth first traversal" algorithm in order to
     * create list of
     * adjacentes for the different level of the tree. During this stage, after reaching a terminal block at level l,
     * the search continues
     * until the level l+1 is reached in order to reach all the remaining (if any) terminal blocks of level l, thus
     * discard longer
     * workflows
     * <li>Workflows building</li>
     * In this stage, the list of adjacentes are traversed from the list of terminal level tl containing one or more
     * terminal blocks, until
     * the list of level 0 containing the root block. Each walk from a terminal block to the root generates a workflow
     * which is then
     * checked; only valid workflows are maintained
     * <li>Shortest workflows selection</li>
     * At the end of the previous stage, there can be several workflows with different lengths generated by different
     * trees (each tree
     * generates workflows of the same length). In this stage only the shortest are selected and returned
     * </ol>
     * <h3>Number of buildable workflows</h3>
     * If not empty, the returned workflows list contains workflows of the same, minimum length. The workflows length is
     * always less than
     * the {@link #getMaxWorkflowLength()}.<br>
     * <br>
     * The exact number of buildable workflows is given by the sum of all the distinct couples (Initial Block, Terminal
     * Block), selecting
     * only:
     * <ol>
     ...
     * </ol>
     * As consequence of (2), built workflows having the same IB (that is owning to the same tree), have always
     * different TB
     * <h5>Examples</h5><br>
     * Given the triples (Initial Block, Terminal Block, Workflow Length) where each couple (IB, TB) is connected<br>
     * <b>Example 1</b>:<br>
     * <ul>
     * <li>Triples: (0,3,7),(0,5,7) - (1,3,7),(1,5,7)</li>
     * <li>4 distinct couples. Since all the workflows have the same length 7, all the 4 workflows are built
     * </ul>
     * <b>Example 2</b>:<br>
     * <ul>
     * <li>Triples: (0,3,5),(0,5,5) - (1,3,7),(1,5,7)</li>
     * <li>4 distinct couples. Since there are 2 workflows of minimum length 5, 2 workflows are built and the others are
     * discarded
     * </ul>
     * <b>Example 3</b>:<br>
     * <ul>
     * <li>Triples: (0,3,5),(0,5,7) - (1,3,7),(1,5,7)</li>
     * <li>4 distinct couples. Since there is a workflow with minimum length 5, one workflow is built and the others are
     * discarded
     * </ul>
     * <b>Example 4</b>:<br>
     * <ul>
     * <li>Triples: (0,3,5),(0,3,5),(0,3,5)</li>
     * <li>1 distinct couple. The same couple (0,3) is connected by 3 different paths of the same length. The sole
     * workflow is selected according to the <i>preference system</i>
     * </ul>
     * <h4>The preference system</h4>
     * The preference system works in two circumstances (not mutually exclusive)<br>
     * <br>
     * <ol>
     * <li>(*) Multiple predecessors</li>
     * A couple (IB, TB) is connected by multiple paths of same length. Considering that the workflow building algoritm
     * walks paths from TB
     * to IB, this happens when a block has more than one direct predecessor. In this case, the <i>preferred</i> block
     * is selected. The
     * preferred block is the block having the best "history preference". The history of a block is given by all its
     * predecessors, directed
     * and non, until the initial block (excluded). The history preference of a block is given by the sum of the {@link
     * Workblock#getPreference()} of its history blocks having a preference set (that is a preference greater than
     * {@link
     * Workblock#MINIMUM_PREFERENCE}. The algorithm threats "brothers" blocks (that is blocks sharing the same
     * father/direct successor
     * block) as a single block having as preference the maximum preference of all the brothers<br>
     * <br>
     * <li>Multiple workflows of same minimum length</li>
     * At the end of the workflows building stage, there can be several workflows of the same minimum length. This can
     * happen if there are
     * multiple connected couples (IB,TB). In this case the preference system is applied by the method {@link
     * #buildPreferred(InputDescriptor, OutputDescriptor)} which takes advantage of the {@link Workflow#getPreference()}
     * method
     * </ol>
     *
     * @param initDescriptor
	...
	printLog("Building STARTED");

	for (Workblock workblock : blockList) {
	    // System.out.println("WB: " + workblock.toString());
	}
	...
	printLog("Blocks count: " + this.blockList.size());
	...
	// System.out.println("INITIALS: " + initials.size());
	// initials.forEach(i -> System.out.println(i));

	findTerminals(output);

	printLog("Found " + initials.size() + " initial blocks");
	printLog("Found " + terminals + " terminal blocks");

	if (deepLogsEnabled) {
	    for (Workblock initial : initials) {
		GSLoggerFactory.getLogger(getClass()).info("Initial block: {}", initial);
	    }
	}
	...
	while (!queue.isEmpty()) {
	    ...
	    // System.out.println("After poll: " + queue.stream().map(b -> b.getId()).collect(Collectors.toList()));
	    ...
	    // System.out.println("After add: " + queue.stream().map(b -> b.getId()).collect(Collectors.toList()));
	}
    ...
    /**
     * Finds a predecessor (if any) of the supplied block contained in the adj list, according to the block with the
     * best history
     * preference. If a block with a a best history preference do not exists, returns a random block
     *
     * @param block
     * @param adjList
