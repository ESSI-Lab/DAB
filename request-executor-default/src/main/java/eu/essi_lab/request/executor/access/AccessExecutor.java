...
public class AccessExecutor extends AbstractAuthorizedExecutor implements IAccessExecutor {
	...
	public ResultSet<DataObject> retrieve(AccessMessage accessMessage) throws GSException {
		...
		if (resultSet.getResultsList().isEmpty()) {
			...
			// only to well known online ids (by the way, the published online should all be
			// conform to the
		...
		Optional<ElasticsearchInfoPublisher> publisher = ElasticsearchInfoPublisher
				.create(accessMessage.getWebRequest());
	 ...
	 */
	@Override
	public ResultSet<DataObject> retrieve(GSResource resource, String onlineId, DataDescriptor targetDescriptor)
			throws GSException {

		ReportsMetadataHandler handler = new ReportsMetadataHandler(resource);

		List<DataComplianceReport> reports = handler.getReports();
		Optional<DataComplianceReport> optReport = reports.stream().filter(r -> r.getOnlineId().equals(onlineId))
				.findFirst();
		...
		// GSLoggerFactory.getLogger(getClass()).info("Retrieving data compliance report
		// ENDED");

		if (downloader == null) {

			throw GSException.createException(getClass(), //
					"Data downloader not found for resource [" + resource.getPrivateId() + "] and online resource ["
							+ onlineId + "]", //
					ErrorInfo.ERRORTYPE_INTERNAL, //
					ErrorInfo.SEVERITY_ERROR, //
		...
		DataObject dataObject = retrieveDataObject(resource, downloader, report.getFullDataDescriptor(),
				targetDescriptor);
	...
	public DataObject retrieveDataObject(GSResource resource, DataDownloader downloader,
		...
		// frequently updated sources, such as real time sensors. In this case updated
		// data
		// is continuously being created, hence the temporal dimension of the descriptor
		// is always increasing)
		List<DataDescriptor> remoteDescriptors = downloader.getRemoteDescriptors();
		// hence, among the obtained remote descriptors, we choose the one that is most
		...
		// In this step we modify the chosen remote descriptor according to the user
		// required target descriptor
		// and the downloader capabilities (e.g. if user has requested a subset of the
		// entire data and the downloader
		// can subset the data, then the remote descriptor is modified setting the
		...
		// to the descriptor
		Optional<DataValidator> opt = DataValidatorFactory.create(remoteDescriptor.getDataFormat(),
				remoteDescriptor.getDataType());
		if (opt.isPresent()) {
			...
			// the validator provides a most accurate descriptor. It might indeed happen
			// that we ask a downloader to
			...
			if (targetDescriptor != null) {
				...
				// e.g. a target descriptor might have only the format field specified if user
				// requests only to download
				// the dataset with a specific format, regardless of the other settings, such as
				// resolution, subset, and
				// so on.
				// the blank fields must be filled, in order for the workflow builder to work.
			...
			if (validatedCRSIdentifier == null || validatedCRSIdentifier.isEmpty()) {
				// the CRS identifier is set from the original CRS identifier, in case where the
				// validator didn't
				// recognized
				// a CRS identifier (in cases where at least projection has been recognized)
				if (validatedCRSIdentifier == null || validatedCRSIdentifier.isEmpty()) {
					// the CRS identifier is set from the original identifier, in case where the
					// validator didn't
					// recognized
					// a CRS identifier (in cases where at least projection has been recognized)
	 ...
	 * @param targetDescriptor
	 */
	private void enhanceRemoteDescriptor(DataDownloader downloader, DataDescriptor fullDescriptor,
			DataDescriptor tmpDescriptor) {
		...
		if (!sourceCRS.equals(targetCRS)) {
			...
			if (spatial1 != null && spatial2 != null) {
				SimpleEntry<Double, Double> lower = new SimpleEntry<Double, Double>(
						spatial1.getContinueDimension().getLower().doubleValue(),
						spatial2.getContinueDimension().getLower().doubleValue());
				SimpleEntry<Double, Double> upper = new SimpleEntry<Double, Double>(
						spatial1.getContinueDimension().getUpper().doubleValue(),
						spatial2.getContinueDimension().getUpper().doubleValue());
				...
				try {
					...
					case EAST_NORTH:
						...
						// (commented because it seems geotools is returning in the correct order
					...
					Double sourceLower1 = fullDescriptor.getFirstSpatialDimension().getContinueDimension().getLower()
							.doubleValue();
					Double sourceUpper1 = fullDescriptor.getFirstSpatialDimension().getContinueDimension().getUpper()
							.doubleValue();
					Double sourceLower2 = fullDescriptor.getSecondSpatialDimension().getContinueDimension().getLower()
							.doubleValue();
					Double sourceUpper2 = fullDescriptor.getSecondSpatialDimension().getContinueDimension().getUpper()
							.doubleValue();

					// the THREDDS case... it has coverages with longitude from 0 to 360 instead of
					// -180 180
					if (sourceCRS.equals(CRS.EPSG_4326()) && Math.abs(sourceUpper2 - 180.0) > TOL
							&& upperTarget2 < TOL) {
						lowerTarget2 += 360.0;
						upperTarget2 += 360.0;
					}
			...
			for (int i = 0; i < fullDescriptor.getSpatialDimensions().size(); i++) {
				...
				if (downloader.canSubset(dataDimension.getName())) { //
					...
					if (targetDimension != null) {
						fullDescriptor.getSpatialDimensions().set(i,
								getTargetDimension(dataDimension, targetDimension));
					}
				}
			}
		}
		...
		if (temporalDimension != null && downloader.canSubset(temporalDimension.getName())) {
			...
			DataDimension targetDimension = getTargetDimension(temporalDimension,
					targetDescriptor.getTemporalDimension());
	...
	private DataDimension getTargetDimension(DataDimension currentDimension, DataDimension requiredDimension) {
		...
		case ABSOLUTE:
			...
			case CONTAINS:
				if (requiredLower != null) {
					// targetLower = difference(requiredLower, lowerTolerance); // possibly more
					// data is asked, this is
					// a
					...
					if (requiredSize == 1) {
						// the lower limit is choosen as the upper if size = 1 and no information about
						// required lower
						// is given
		...
		case MAXIMUM:
		case MINIMUM:
			GSLoggerFactory.getLogger(getClass()).error("This shouldn't happen: " + currentLowerType
					+ " is only to be used in data requests, not in capabilities");
			break;
		default:
			GSLoggerFactory.getLogger(getClass()).error("This shouldn't happen: unexpected case");
			break;
		...
		case ABSOLUTE:
			...
			case CONTAINS:
				if (requiredUpper != null) {
					// targetUpper = sum(requiredUpper, upperTolerance); // possibly more data is
					// asked, this is a
		...
		case MAXIMUM:
		case MINIMUM:
			GSLoggerFactory.getLogger(getClass()).error("This shouldn't happen: " + currentUpperType
					+ " is only to be used in data requests, not in capabilities");
			break;
		default:
			GSLoggerFactory.getLogger(getClass()).error("This shouldn't happen: unexpected case");
