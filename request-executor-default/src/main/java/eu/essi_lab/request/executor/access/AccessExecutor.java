...
public class AccessExecutor extends AbstractAuthorizedExecutor implements IAccessExecutor {
	...
	if (resource != null) {
	    ...
	    for (DataComplianceReport tmpReport : reports) {
		String id = tmpReport.getOnlineId();
		if (!id.equals(onlineId)) {
		    continue;
		}
	...
	DataDescriptor reportDescriptor = null;
	if (report != null) {
	    reportDescriptor = report.getFullDataDescriptor();
	}

	DataObject dataObject = retrieveDataObject(resource, downloader, reportDescriptor, targetDescriptor);
