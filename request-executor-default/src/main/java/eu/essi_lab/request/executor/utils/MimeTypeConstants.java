...
public class MimeTypeConstants {
    ...
    //private static final String s_strDefaultMimeType = "bin";
    protected static Map<String, String> mapContentTypes = null;

    /**
     * Method getting particular Content type (key) and return the extension (value)
     * 
     * @param key - key value Content Type
	...
	String ctype = mapContentTypes.get(key);

	if (ctype == null || ctype.trim().length() == 0) {
	    if (key.contains(";")) {
		String splittedKey = key.split(";")[0];
		ctype = mapContentTypes.get(splittedKey);
		if (ctype == null || ctype.trim().length() == 0) {
		    return null;
		} else {
		    strMimeType = ctype.toString();
		}
	    }
	} else {
	    strMimeType = ctype.toString();
	...
	mapContentTypes.put("application/json", "json");
