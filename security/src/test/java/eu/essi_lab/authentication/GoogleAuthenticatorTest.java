...
/**
 * Google authenticator test check requests made by users who wants to login
 * with google by oauth2 protocol to get an access token back.
 * 
 * @author pezzati
 */
public class GoogleAuthenticatorTest {
    ...
    private GoogleOAuth2Authenticator gAuth;
	...
	gAuth = new GoogleOAuth2Authenticator();
	setUpInvalidConf();
	setUpValidConf();
...
//	exEx.expectMessage("Configuration is null or empty.");
	...
	exEx.expect(GSException.class);
	gAuth.handleLogin(null, httpResponse, null);
	Assert.fail();
    }
...
//	exEx.expectMessage("Service provider callback has no code.");
	gAuth.handleCallback(httpRequest);
	Assert.fail();
    }
	...
	gAuth.initialize(validConf);
	gAuth.setHttpClient(httpClient);
