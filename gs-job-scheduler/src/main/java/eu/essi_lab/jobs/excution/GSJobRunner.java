package eu.essi_lab.jobs.excution;

/*-
 * #%L
 * Discovery and Access Broker (DAB) Community Edition (CE)
 * %%
 * Copyright (C) 2021 National Research Council of Italy (CNR)/Institute of Atmospheric Pollution Research (IIA)/ESSI-Lab
 * %%
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 * 
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 * 
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 * #L%
 */

import static eu.essi_lab.jobs.listener.GSJobListener.GS_JOB_STATUS_EXECUTION_ID;

import java.util.List;
import java.util.Optional;

import org.quartz.JobDataMap;
import org.slf4j.Logger;

import eu.essi_lab.configuration.ConfigurationUtils;
import eu.essi_lab.jobs.GSJobStatus;
import eu.essi_lab.jobs.configuration.IGSConfigurableJob;
import eu.essi_lab.jobs.report.GSJobEventCollectorFactory;
import eu.essi_lab.jobs.report.GSJobStatusCollector;
import eu.essi_lab.jobs.scheduler.quartz.GSQuartzJob;
import eu.essi_lab.lib.utils.GSLoggerFactory;
import eu.essi_lab.model.SharedRepositoryInfo;
import eu.essi_lab.model.exceptions.DefaultGSExceptionHandler;
import eu.essi_lab.model.exceptions.DefaultGSExceptionLogger;
import eu.essi_lab.model.exceptions.DefaultGSExceptionReader;
import eu.essi_lab.model.exceptions.ErrorInfo;
import eu.essi_lab.model.exceptions.GSException;
import eu.essi_lab.shared.messages.SharedContentReadResponse;
import eu.essi_lab.shared.model.SharedContent;
public class GSJobRunner implements GSJobExecutionPhase {

    private transient Logger logger = GSLoggerFactory.getLogger(GSJobRunner.class);

    private static final String UNKNOWN_GSJOBEXECUTIONEXECPTION_ERR_ID = "UNKNOWN_GSJOBEXECUTIONEXECPTION_ERR_ID";

    Optional<GSJobStatus> findExistingJobstatus(JobDataMap map, String jobid) {
	try {

	    SharedRepositoryInfo repo = ConfigurationUtils.getSharedRepositoryInfo();

	    GSJobStatusCollector collector = GSJobEventCollectorFactory.getGSJobStatusCollector(repo);

	    String existingid = (String) map.get(GS_JOB_STATUS_EXECUTION_ID);

	    SharedContentReadResponse<GSJobStatus> response = collector.read(existingid);

	    List<SharedContent<GSJobStatus>> contents = response.getContents();

	    if (!contents.isEmpty()) {
		return Optional.ofNullable(contents.get(0).getContent());
	    }

	} catch (GSException e) {

	    logger.warn("GSException retrieving job status for job id {}", jobid);

	    DefaultGSExceptionLogger.log(new DefaultGSExceptionHandler(new DefaultGSExceptionReader(e)));

	} catch (Exception e) {

	    logger.warn("Alien Exception retrieving job status for job id {}", jobid, e);

	}

	return Optional.empty();
    }

    @Override
    public GSJobExecutionPhaseResult doPhase(IGSConfigurableJob job, JobDataMap map, Boolean recovering) {

	logger.info("Start actual run of {}", job.getId());

	GSException gse;

	try {

	    Optional<GSJobStatus> jobStatus = findExistingJobstatus(map, job.getId());

	    job.run(map, recovering, jobStatus);

	    if (jobStatus.isPresent()) {

		Optional<GSJobStatusCollector> optionalCollector = getJobStatusCollector();

		if (optionalCollector.isPresent()) {

		    GSJobStatusCollector collector = optionalCollector.get();

		    collector.store(jobStatus.get());

		}

	    }

	    return GSJobExecutionPhaseResult.PASS;

	} catch (GSException e) {

	    logger.error("GSException executing job {}", job.getId());

	    DefaultGSExceptionLogger.log(new DefaultGSExceptionHandler(new DefaultGSExceptionReader(e)));

	    gse = e;

	} catch (Exception e) {

	    logger.error("Alien Exception executing job {}", job.getId());

	    gse = GSException.createException(//
		    GSQuartzJob.class, //
		    "Unknown error during batch job execution", //
		    null, //
		    ErrorInfo.ERRORTYPE_INTERNAL, //
		    ErrorInfo.SEVERITY_ERROR, //
		    UNKNOWN_GSJOBEXECUTIONEXECPTION_ERR_ID, //
		    e);

	    DefaultGSExceptionLogger.log(new DefaultGSExceptionHandler(new DefaultGSExceptionReader(gse)));
	}

	map.put(GSJobExecutor.JOB_EXECUTION_EXCEPTION_ID, gse);

	return GSJobExecutionPhaseResult.STOP;

    }

    public Optional<GSJobStatusCollector> getJobStatusCollector() throws GSException {
	SharedRepositoryInfo repo = ConfigurationUtils.getSharedRepositoryInfo();

	return Optional.of(GSJobEventCollectorFactory.getGSJobStatusCollector(repo));

    }

    @Override
    public String toString() {
	return "GS Job Runner Phase";
    }
}
