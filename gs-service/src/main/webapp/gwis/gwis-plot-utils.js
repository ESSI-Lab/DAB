...
// these defaults are applied for unspecified input plot options when creating a
// new plot
// specified input plot options override these defaults
// built-in api defaults are used when neither these default options nor the
...
GWIS._nwisRootUrl = "../../services/nwis"; // https enabled 12/14/2016
...
//
// output:
// when creating plot in a document div, the dygraphs plot object is returned
// that can be manually manipulated (see:
...
GWIS._checkOpts = function _checkOpts( opts ) {
    ...
    var checkOpt = function( opt_obj, opt_name, opt_type, opt_default ) {
        opt_name = opt_name.toLowerCase();                                           // case
																						// insensitive
        input_list = $.grep(input_list, function(val) { return val !== opt_name; }); // remove
																						// from
																						// input
																						// list
        if (opt_obj[opt_name]===undefined)   { opt_obj[opt_name] = opt_default; }    // set
																						// default
        if (opt_obj[opt_name]===undefined && opt_default===undefined) { return; }    // final
																						// opt
																						// and
																						// default
																						// undefined
																						// (ok)
        if (opt_type==="number") {                                                   // try
																						// to
																						// parse
																						// string
																						// to
        ...
        if (opt_type==="boolean" && ( opt_obj[opt_name]===1 || /^\s*T/i.test(opt_obj[opt_name]+"") )) { opt_obj[opt_name] = true;  } // support
																																		// 1 or
																																		// "t..."
																																		// as
																																		// true
																																		...
																																		// bool
																																		// types
        if (opt_type==="boolean" && ( opt_obj[opt_name]===0 || /^\s*F/i.test(opt_obj[opt_name]+"") )) { opt_obj[opt_name] = false; } // support
																																		// 0 or
																																		// "f..."
																																		// as
																																		// false
    ...
    // set width and/or height null to use default width and/or height based on
    ...
    // whether to close all other popup plots opened by GWIS before opening this
	// popup
    // has no effect when 'div_id' used to create plot in the document
    ...
    // whether to plot instantaneous values (IV, real-time and historical) or
	// daily values (DV)
    ...
    // whether to express times for iv plots in your local time zone ("local")
	// or utc ("utc")
    // because plots can contain data from many time zones, a common time zone
	// must be chosen when data has a time component
    ...
    // ......................................
    // opts.period [string]
    // time period from past to now for the data to plot, ISO-8601 duration
	// format
    // see: https://en.wikipedia.org/wiki/ISO_8601#Durations
    // the time period to plot can be defined by specifying EITHER "period" OR
	// both "start_dt" and "end_dt"
    ...
    if (
        ( opts.period   !== undefined && (opts.start_dt !== undefined || opts.end_dt !== undefined) ) || // period
																											// with
																											// start_dt
																											// and/or
																											// end_dt
        ( opts.start_dt !== undefined &&  opts.end_dt   === undefined                               ) || // start_dt
																											// without
																											// end_dt
        ( opts.end_dt   !== undefined &&  opts.start_dt === undefined                               )    // end_dt
																											// without
																											// start_dt
    ) {
    ...
    // auto_refresh can only be set true when plotting instantaneous values
	// ('iv_or_dv'='iv') and the 'period' time period option is used
    ...
    // array of plot line definitions that determine the data and appearance of
	// the lines added to the plot
    // array must contain at least 1 plot line definition
    // example syntax:
    // [
    // { site:"08211000", pcode:"00060" }, // minimum options
    // { site:"08208000", pcode:"00065", label:"Gage Height", axis:"y2",
	// color:"purple", fill_graph:true, stroke_width:5, draw_points:true,
	// point_size:5 },
    ...
    // a negative percentage shows that percentage of the full range starting at
	// the beginning,
    ...
    // line color to use for the interactive range selector mini plot below main
	// plot
    // css-style color definition, such as "blue", "#00f", "rgb(0,0,255)",
	// "rgba(0,0,255,0.5)", etc
    ...
    // fill color to use for the interactive range selector mini plot below main
	// plot
    // has no effect when 'range_selector' is false
    ...
    // either:
    // true - fix the y-axis range to the full range of the data regardless of
	// the currently viewed x-range
    // ie: fixes y-scale to full y-range when panning
    // false - dynamically adjust the y-axis to fill the plot with the currently
	// viewed data
    // ie: y-axis generally changes to fill available space as the x-axis is
	// zoomed
    ...
    // ......................................
    // opts.controls [array]
    // array of strings containing which controls to add at the bottom of the
	// plot
    // control are ordered left-to-right under the plot in the same order as in
	// the array
    ...
    // "y_fixscale" - (checkbox) toggles whether the y-scale is fixed to the
	// full y-range
    ...
    // the logscale control is not shown if any plot data if less than or equal
	// to 0
    ...
    // function can take single input arg "plot" (dygraphs plot object) to
	// access the plot if needed
    checkOpt( opts, "on_success", "function", function(){} );
    
    // ......................................
    // opts.on_error [function]
    // function to execute if plot not successfully created
    // function can take single input arg "msg" to access the error message if
	// needed
    checkOpt( opts, "on_error", "function", function(){} );
    
    // ......................................
    // opts.on_complete [function]
    // function to execute when plot creation is complete, either upon success
	// or error
    ...
    // executes upon creation when plot data is first populated, and any
    ...
    // whether to write api status messages to developer console
    // sets global api verbose mode, so setting true/false turns verbose mode
	// on/off for all subsequent api interaction
    ...
    // see if any unrecognized user input options are left after removing above
	// valid options
    ...
    $.each( opts.series, function(idx) {
        ...
        // ...add any options defined in opt_defaults.series not specified in
		// input...
...
// if ( ser.site===undefined ) { throw new Error(funcName+"series option 'site'
// is required to specify what site to retrieve data for"); }
// if ( /\D/.test(ser.site) ) { throw new Error(funcName+"series option 'site'
// must be all digits"); }
  ...
  // checkOpt( ser, "pcode", "string", undefined );
   // if ( ser.pcode === undefined ) { throw new Error(funcName+"series option
	// 'pcode' is required to specify what site data to retrieve and plot"); }
// if ( !/^\d{5}$/.test(ser.pcode) ) { throw new Error(funcName+"series option
// 'pcode' must be 5 digits"); }
        
        // ......................................
        // dv_stat [string]
        // when plotting daily values ('iv_or_dv' = 'dv'), the statistic to use
        // daily values are computed from instantaneous values using various
		// statistics
        ...
        checkOpt( ser, "dv_stat", "string", ( opts.iv_or_dv==="dv" ? "00003" : undefined ) ); // default
																								// depends
																								// on
																								// iv_or_dv
// if (ser.dv_stat && !/^[0-9]{5}$/.test(ser.dv_stat)) {
// throw new Error(funcName+"series option 'dv_stat' must be a 5-digit daily
...
// }
        ...
        // ..............
        // color [string]
        // line color specified as a css-style color, such as "blue", "#00f",
		// "rgb(0,0,255)", "rgba(0,0,255,0.5)", etc
        checkOpt( ser, "color", "string", undefined ); // dygraphs chooses
														// unique color when
														// undefined
        ...
        // ..............
        // fill_graph [boolean]
        // whether to create an fill plot, where the area between the line and
		// y=0 axis is colored
        // if the line crosses the y=0 axis, the fill area will not be draw
        // the fill area is above the line when the line is below the y=0 axis
		// (eg: all negative values)
        checkOpt( ser, "fill_graph", "boolean", false ); 
        
        // ..............
        // see if any unrecognized user input options are left after removing
		// above valid options
// if (input_list.length > 0) {
...
GWIS._popup = function( opts ) {
    ...
    // 2-element array of popup [widthPx,heightPx], use default for null array
	// values
    var w = opts.popup_size[0]; if (w === null) { w = Math.ceil( 0.7 * $(window).width()  ); } // default:
																								// 70%
																								// browser
																								// width
    var h = opts.popup_size[1]; if (h === null) { h = Math.ceil( 0.4 * $(window).height() ); } // default:
																								// 40%
																								// browser
																								// height
    ...
    GWIS._popups = $.grep(GWIS._popups, function(p) { return p !== null; }); // remove
																				// nulls
    ...
    var popup = window.open(
        ...
        "_blank",  // new window
        // ...options...
        "toolbar=no,"     +   // whether or not to display the browser toolbar
								// (default=yes)
        "location=no,"    +   // whether or not to display the address field
								// (default=yes)
        "directories=no," +   // whether or not to add directory buttons
								// (default=yes, IE ONLY)
        "status=no,"      +   // whether or not to add a status bar
								// (default=yes)
        "menubar=no,"     +   // whether or not to display the menu bar
								// (default=yes)
        "scrollbars=yes," +   // whether or not to display scroll bars
								// (default=yes)
        "resizable=yes,"  +   // whether or not the window is resizable
								// (default=yes)
        "copyhistory=no," +   // whether to copy history
        "width="  + w + "," + // width of window in pixels (min values are
								// 100)
        "height=" + h +       // height of window in pixels (min values are
								// 100)
    ...
    popup.document.write(
        ...
        '<html>' +
            ...
            '<body>' +
                ...
                '<script>' +
                    'setTimeout( function() {' + // time for plot div to size
													// correctly
                        'try {' +
                            'var opts = window.opener.GWIS._popupOpts;' + // get
																			// opts
																			// from
																			// parent
                            'opts.div_id   = "plot";'    +                // set
																			// plot
																			// div
                            'window.plot = GWIS._createPlot( opts );'  +  // create
																			// plot
                            'setInterval( function(){ if (typeof plot.resize==="function"){ plot.resize();} }, 500);' + // IE:
																														// popups
																														// don't
																														// seem
																														// to
																														...
																														// on
																														// timer
                            'delete window.opener.GWIS._popupOpts;' +     // cleanup
																			// temp
																			// parent
																			// opts
                        '} catch(err) {' +
                            'GWIS._plotError( opts, err.message );' +     // catch
																			// error
                        '}' +
                    '}, 100);' +
                '</script>' +
    ...
    // cannot find reliable cross-browser way to immediately return plot obj
	// from popup - return nothing
...
GWIS._createPlot = function ( opts ) {
    ...
    $("#"+opts.div_id)
        ...
        .removeClass("gwis-error") // remove any error class in case previous
									// error and new plot being created
        .append(
            // ...loading overly...
    ...
    $.each( opts.series, function(idx,ser) {
        ...
        series[ser.label] = {
            ...
            "drawPoints"          : ser.draw_points,    // whether to draw data
														// circles at each data
														// point on the line
            "pointSize"           : ser.point_size,     // size of the circles
														// to draw at data
														// points
            "fillGraph"           : ser.fill_graph,     // whether to fill area
														// under plot line
            "showInRangeSelector" : true,               // whether to show in
														// range selector
        };
    });
    ...
    var plot = new Dygraph(
        ...
        { // options
            ...
            // ...axes...
            "labelsUTC" : (opts.iv_or_dv==="iv" ? (opts.iv_local_or_utc==="local" ? false : true ) : true ), // whether
																												// to
																												// show
																												// datetime
																												// labels
																												...
																												// or
																												// iv
            "xRangePad"   : 2,               // extra buffer space around
												// x-range [px]
            "yRangePad"   : null,            // extra buffer space around
												// y-range [px] - needs to be
												// null if allowing logscale
            "includeZero" : opts.y_show0,    // whether to always show 0 on
												// y-axes (both)
            "logscale"    : false,           // whether to logscale all axes
												// (time axes are ignored, eg:
												// x-axis) - set using opts
												// after data range found
            
            // ...legend...
            "legend"              : opts.legend, // legend display option:
													// "never", "always",
													// "onmouseover"
            "labelsSeparateLines" : true,        // whether to put line
													// breaks between lines in
													// the label string
            
            // ...data lines...
            "drawGapEdgePoints" : false, // draw points at the edges of gaps
											// in the data
            
            // ...range selector...
            "showRangeSelector"            : opts.range_selector,                  // whether
																					// to
																					// show
																					// range
																					// selector
																					...
																					// main
																					// plot
            "rangeSelectorHeight"          : 60,                                   // height
																					// of
																					// range
																					// selector
            "rangeSelectorPlotStrokeColor" : opts.range_selector_stroke,           // range
																					// selector
																					// mini
																					// plot
																					// stroke
																					// color
            "rangeSelectorPlotFillColor"   : opts.range_selector_fill,             // range
																					// selector
																					// mini
																					// plot
																					// fill
																					// color
            "interactionModel"             : Dygraph.defaultInteractionModel,      // adding
																					// range
																					// selector
																					// turns
																					// off
																					...
																					// back
																					// on
            "animatedZooms"                : (opts.range_selector ? false : true), // whether
																					// to
																					// animate
																					// zooms
																					// (cannot
            ...
            "axes": {
                "x": {
                    "drawGrid"       : opts.x_grid,   // whether to display
														// gridlines
                    "pixelsPerLabel" : 70,            // #px between each axis
														// ticks (larger values
														// make sparser ticks
														// and labels)
                    "valueFormatter" : function(ms) { // format function for
														// value displayed in
														// legend
                        var d = new Date(ms);
    ...
    plot.full_screen = function() {
        ...
        if ( $("#"+plot.opts.div_id).hasClass("gwis-fullscreen") ) {
            ...
            // user can manually exit (eg: ESC) - set exit full screen event
			// handler
            $(document).on("fullscreenchange webkitfullscreenchange mozfullscreenchange MSFullscreenChange", function() {
                if ( document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement ) { return; } // do
																																									// nothing
																																									// if
																																									// in
																																									// full
    ...
    // when true, the y-axes are fixed to the full y-range of all available
	// data, not just the y-range of the data in the currently viewed time
	// window
    // useful when panning the x-axes and you do not want the y-range to
	// auto-adjust to fit the currently viewed data
    ...
    plot.y_fixscale = function(true_false) {
        ...
        // get y1 range to use
        var ymin; var ymax;
        ymin = plot.data_y1min; ymax = plot.data_y1max;           // natural
																	// range of
																	// data
        ymin = ymin-0.1*(ymax-ymin); ymax = ymax+0.1*(ymax-ymin); // add 10%
																	// buffer
        if ( plot.getOption("includeZero")===true && ymin> 0 ) { ymin = 0;      } // reset
																					// 0 if
																					// y_show0
																					// set
																					// (line
																					// above
																					// y=0)
        if ( plot.getOption("includeZero")===true && ymax< 0 ) { ymax = 0;      } // reset
																					// 0 if
																					// y_show0
																					// set
																					// (line
																					// below
																					// y=0)
        if ( plot.getOption("logscale")   ===true && ymin<=0 ) { ymin = 0.0001; } // reset
																					// small
																					// if
																					// logscale
        ...
        plot.updateOptions({
            "axes":{
                "y" : { "valueRange": ( true_false ? valueRange1 : null ) }, // apply
																				// if
																				// checked,
																				// unset
																				// if
																				// not
																				// checked
                "y2": { "valueRange": ( true_false ? valueRange2 : null ) }  // apply
																				// if
																				// checked,
																				// unset
																				// if
        ...
        $("#"+plot.opts.div_id+"_checkbox_fixedY").prop("checked",true_false); // set
																				// control
																				// checkbox
    ...
    // if either or both y-axis contains data less than or equal to 0, throws a
    ...
    plot.y_logscale = function(true_false) {
        ...
        if ( Math.min( min_max ) <= 0 ) {
            plot.updateOptions({ "logscale" : false });                    // no
																			// plot
																			// logscale
            $("#"+plot.opts.div_id+"_checkbox_logscale").parent().hide();  // hide
																			// control
        ...
        $("#"+plot.opts.div_id+"_checkbox_logscale").prop("checked",true_false); // set
																					// control
																					// checkbox
        var axOpts = plot.getOption("axes"); if (axOpts.y.valueRange !== null) { plot.y_fixscale(true); } // y_fixscale
																											// on -
																											// need
																											// reset
																											// in
    ...
    plot.y_show0 = function(true_false) {
        ...
        $("#"+plot.opts.div_id+"_checkbox_Yshow0").prop("checked",true_false); // set
																				// control
																				// checkbox
        var axOpts = plot.getOption("axes"); if (axOpts.y.valueRange !== null) { plot.y_fixscale(true); } // y_fixscale
																											// on -
    ...
    plot.x_grid = function(true_false) {
        ...
        // set
        plot.updateOptions({ "axes":{"x":{"drawGrid":true_false}} }); // set
																		// option
        $("#"+plot.opts.div_id+"_checkbox_Xgrid").prop("checked",true_false); // set
																				// control
																				// checkbox
    ...
    plot.y_grid = function(true_false) {
        ...
        $("#"+plot.opts.div_id+"_checkbox_Ygrid").prop("checked",true_false); // set
																				// control
																				// checkbox
    ...
    // refresh
    // refresh the plot data from USGS Water Services using same data options
    ...
    // remove
    // permanently remove the plot, clearing the plot div and cleaning up
	// resources
    plot.remove = function() {
        ...
        this.destroy();        // dygraphs method - detaches dom elements and
								// nulls out all data references
...
GWIS._refreshPlot = function (plot) {
    ...
    // cannot do single call because site_no-pcode pairs are distinct for each
	// plot line
    var rdb = "";
    var n_complete = 0;
    $.each( plot.opts.series, function(idx,ser) {
        ...
        // service call
        GWIS._console(funcName+"linkage="+ser.linkage+" protocol='"+ser.protocol+"' name='"+ser.name+"': requesting data from water services");
        $.ajax({
            "type"     : "GET",
            "url"      : url+"&linkage="+ser.linkage + "&protocol="+ser.protocol + "&name="+ser.name + (ser.dv_stat!==undefined ? "&statCd="+ser.dv_stat : ""), // add
																																	// site,
																																	// pcode,
																																	// and
																																	// dv_stat
            ...
            "error": function(err) {
                ...
                // service error
                GWIS._console(funcName+"': water services request failed - series not included in plot: "+err.statusText,"warn");
            ...
            "success": function(txt) {
                ...
                if ( /^#  No sites found/.test(txt) ) {
                    // no data
                    GWIS._console(funcName+"': water services request returned no data - series not shown in plot","warn");
                } else {
                    // have some data - add to combined rdb
                    GWIS._console(funcName+"': water services request returned data");
                    rdb += txt;
                }
            ...
            "complete" : function() {
                ...
                n_complete++;
                if (n_complete < plot.opts.series.length) { return false; }
                ...
                try {
                    ...
                    // trigger events
                    plot.opts.on_success( plot );  plot.opts.on_success  = function(){}; // trigger
																							// on_success
																							// (takes
																							// plot
																							// as
																							...
																							// and
																							// disconnect
                    plot.opts.on_complete();       plot.opts.on_complete = function(){}; // trigger
																							// on_complete
																							// and
																							// disconnect
                    plot.opts.on_refresh( plot ); // trigger on_refresh (takes
													// plot as input arg) and
													// keep connected
...
GWIS._parseRDB = function ( plot, rdb ) {
    ...
    // |# comment lines
    // |agency_cd site_no datetime tz_cd 140099_00065 140099_00065_cd
	// 140101_00065 140101_00065_cd
    ...
    var Y    = {};  // obj of data for each series - these need to be combined
					// into single plot data array lined up in time
    var Icol = {};  // column index lookup
    var pcode_stat; // parsed data uniquely identified by "site_pcode_stat" (dv)
					// or "site_pcode" (iv)
    $.each( rdb.split(/\n/), function(idx,line) { // loop through lines
        ...
        if ( /^agency_cd/.test(line) ) {
            ...
            if ( /tz_cd/.test(line) ) {
                // IV - has tz_cd
                // ...set value col...
                // temp = $.grep(vals, function(val) { return
				// /^[0-9]+_[0-9]+$/.test(val); }); // TSID_PCODE
                // Icol["val"] = $.inArray(temp[0],vals); // if multiple, only
				// get
														// 1st one
            	Icol["val"] = 4;
                // ...set code col...
                // temp = $.grep(vals, function( val ) { return
				// /^[0-9]+_[0-9]+_cd$/.test(val); }); // TSID_PCODE_CD
                // Icol["code"] = $.inArray(temp[0],vals); // if multiple, only
				// get
														// 1st one
            	Icol["code"] = 5;
            } else {
                ...
                Icol["val"] = $.inArray(temp[0],vals); // if multiple, only get
														// 1st one
                // ...set code col...
                temp = $.grep(vals, function( val ) { return /^[0-9]+_[0-9]+_[0-9]+_cd$/.test(val); }); // TSID_PCODE_STAT_CD
                Icol["code"] = $.inArray(temp[0],vals); // if multiple, only get
														// 1st one
            }
            ...
            Icol["tz"  ] = $.inArray( "tz_cd",    vals ); // -1 (missing) if
															// DV
            ...
            pcode_stat = vals[Icol["val"]].replace(/^[0-9]+_/,""); // get rid
																	// of tsid
        ...
        if (tz !== undefined) {
            // IV
            date = date.replace(" ","T"); // "2016-09-26 09:45" =>
											// "2016-09-26T09:45"
            switch(tz.toUpperCase()) {    // add tz offset based on tz_cd for
											// cross-browser
											// 'yyyy-mm-ddThh:mm+XX:XX'
											// Date.parse format
                // offsets from:
				// http://nwis.usgs.gov/nwisdocs4_7/qw/QW-AppxJ.pdf
                ...
                case "ZP6":   // ZP6 GMT +6 hours +06:00
                    date += "+06:00"; break;
                case "GST":   // GST Guam Standard Time, Russia Zone 9 +10:00
								// (not "common" but in CWIS)
                    date += "+10:00"; break;
            ...
            // DV
            date = date + "T12:00-00:00"; // "2016-09-26" =>
											// "2016-09-26T12:00-00:00" (UTC
											// applied at noon)
        }
        ...
        Y[series_id]["data"][utc_ms] = { "y":num, "c":code }; // add code in
																// case we want
																// option to
																// symbolize on
																// code later
    ...
    if ( T.length <= 1 ) { throw new Error(funcName+T.length+" time series points parsed from water services response - at least 2 are required for plotting"); }
    ...
    // cannot reliably do this for iv since collection times are potentially
	// random
    ...
    $.each( plot.opts.series, function(idx,ser) {
        // var series_id =
		// ser.site+"_"+ser.pcode+(ser.dv_stat?"_"+ser.dv_stat:""); //
		// site_pcode_stat
    ...
    plot.rawData_ = []; // rawData_ is where dygraphs stores its native data
						// array - put final data there
...
if (GWIS._isLoadedJq) { $(window).resize( GWIS._center_mes ); } // execute on
																// window resize
...
GWIS._console = function _console( msg, mode ) {
    ...
    if (mode === "D" && !GWIS.verbose) { return 0; } // no debug log if
														// verbose not true
...
// ===========================================================
// BEGIN opensource dygraphs api
// downloaded: http://dygraphs.com/download.html
// date: 9/23/2016
// version: 1.1.1
...
/*
 * ! @license Copyright 2014 Dan Vanderkam (danvdk@gmail.com) MIT-licensed
...
// ===========================================================
// END opensource dygraph api
// ===========================================================

// globally disable touch - it interfers when using api in mobile devices
