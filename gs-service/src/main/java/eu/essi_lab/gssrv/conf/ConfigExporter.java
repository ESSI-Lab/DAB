 ...
 * @author Fabrizio
 */
public class ConfigExporter extends ComponentInfo {
	...
	setComponentName("Configuration export");
	...
	TextField targetLocationField = new TextField();
	targetLocationField.setPlaceholder("Target directory (e.g.: D:/configuration)");
	targetLocationField.setValue("");
	targetLocationField.setWidthFull();
	targetLocationField.getStyle().set("margin-left", "10px");
	...
	exportButton.addClickListener(event -> {
		...
		DatabaseSource source = DatabaseSource.of("");

		try {

		    ClonableInputStream configStream = new ClonableInputStream(source.getStream());

		    FileUtils.copyInputStreamToFile(configStream.clone(), new File(targetLocationField.getValue()));
	...
	exportLayout.add(targetLocationField);
	...
	settingComponent.getOptionTextFields("OpenSearch")
		.forEach(tf -> tf.addValueChangeListener(event -> checkFields(msgDiv, exportButton, setting)));
	...
	radioGroup.addValueChangeListener(event -> checkFields(msgDiv, exportButton, setting));
	...
	verticalLayout.add(exportLayout);

	verticalLayout.add(settingComponent);

	checkFields(msgDiv, exportButton, setting);
	...
	TabDescriptor tabDescriptor = TabDescriptorBuilder.get().//
		withIndex(GSTabIndex.CONFIGURATION_EXPORTER.getIndex()).//
     ...
     * @param setting
     */
    private void checkFields(Div label, Button exportButton, ConfigurationSourceSetting setting) {

	Optional<StorageInfo> info = setting.getSelectedStorageInfo();

	exportButton.setEnabled(info.isPresent());
