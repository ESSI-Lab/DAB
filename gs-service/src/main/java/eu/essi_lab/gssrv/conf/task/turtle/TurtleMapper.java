...
public class TurtleMapper extends DiscoveryResultSetMapper<String> {
	...
	public String map(DiscoveryMessage message, GSResource resource) throws GSException {
		try {
			...
			String myDataset = "<https://dataset.geodab.eu/dataset/"
					+ urlEncode(resource.getSource().getUniqueIdentifier()) + "/" + urlEncode(filename + ".ttl") + ">";
			...
			for (int i = 0; i < onlines.size(); i++) {
				...
				FAIREaseMapping mapping = FAIREaseMapper.map(protocol, linkage,
...
//	}
	...
	private List<SimpleEntry<String, String>> getKeywordsURIs(XMLDocumentReader reader, String type)
			throws XPathExpressionException {
		List<SimpleEntry<String, String>> ret = new ArrayList<>();
		Node[] keywords = reader
				.evaluateNodes("//*:MD_Keywords[*:type/*:MD_KeywordTypeCode/@codeListValue='" + type + "']/*:keyword");
		for (Node keyword : keywords) {
