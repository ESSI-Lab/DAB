...
public class DeploymentUpdaterTask extends AbstractCustomTask {
	...
	public void doJob(JobExecutionContext context, SchedulerJobStatus status) throws Exception {
		...
		for (String line : lines) {
			...
			if (decoded == null) {
				GSLoggerFactory.getLogger(getClass()).error("unexpected settings for updater task. Expected: "
						+ DeploymentUpdaterTaskKey.getExpectedKeys());
				return;
			}
	...
	public static void setDeploymentsToSources(OpenSearchDatabase db, String[] sources, String[] deployments)
		...
		for (String sourceId : sources) {
			...
			String dataFolderTag = db.getMetaFolders().stream().map(dir -> (OpenSearchFolder) dir)
					...
					map(dir -> {
						try {
							return dir
									.getSourceWrapper(
											DatabaseFolder.computeSourceId("production", dir.getName()) + "_dataFolder")
									.getDataFolder().get();
						} catch (Exception e) {
			...
			GSLoggerFactory.getLogger(DeploymentUpdaterTask.class).info("Data folder size: {}",
			...
			while (processedRecords < size) {
				...
				SearchResponse<Object> response = wrapper.search(
						...
						Optional.of(SortOrder.ASCENDING), // sortOrder
