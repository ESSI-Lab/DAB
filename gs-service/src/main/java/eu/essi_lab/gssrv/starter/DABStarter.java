...
import eu.essi_lab.dab.sparql.SparqlProxyServlet;
	...
	*/
		...
		try {
			...
			if (configURL == null || configURL.isEmpty()) {
				...
				GSLoggerFactory.getLogger(DABStarter.class)
						.warn("Configuration URL not found, using fallback URL: local FS temp");
				configURL = "file:temp";
			}
			...
			if (DatabaseSourceUrl.check(configURL)) {
				...
				if (configURL.startsWith("file:temp")) {
					...
					GSLoggerFactory.getLogger(DABStarter.class)
							.info("Local FS configuration on the temp user directory");
			...
			if (mode == ExecutionMode.LOCAL_PRODUCTION) {
				...
				GSLoggerFactory.getLogger(DABStarter.class)
						.info("Creating local config with VOLATILE job store STARTED");
	...
	private void initJaxb() throws GSException {
		...
		try {
			...
			GSLoggerFactory.getLogger(DABStarter.class).error("Fatal error on startup, JAXB could not be initialized",
					e);
	...
	private void healthCheckTest() throws GSException {
		...
		if (skipHealthCheck != null && skipHealthCheck.equals("true")) {
			...
			GSLoggerFactory.getLogger(DABStarter.class)
					.info("Skipping health check according to system variable 'skip.healthcheck'");
			return;
		}
	...
	private void startSchedulerLate() throws GSException {
		...
		if (keyValueOptions.isPresent()) {
			...
			schedulerStartDelay = Integer
					.valueOf(keyValueOptions.get().getProperty(KeyValueOptionKeys.SCHEDULER_START_DELAY.getLabel(),
		...
		new Timer().schedule(new TimerTask() {
			...
			public void run() {
				...
				try {
					...
					GSLoggerFactory.getLogger(DABStarter.class).info("JobStore type: {}",
							schedulerSetting.getJobStoreType());
