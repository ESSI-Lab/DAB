 ...
 * @author Fabrizio
 */
public class ParallelMarkLogicStressTest {
    ...
    //
    //
    private static final int DEFAULT_PARALLEL_QUERIES = 32;
    //
    //
    //
     ...
     * 
     */
    public ParallelMarkLogicStressTest() {

	setParallelQueries(DEFAULT_PARALLEL_QUERIES);
     ...
     * @param queries
     */
    public void setParallelQueries(int queries) {

	this.parallelQueries = queries;
    }
	...
	TaskListExecutor<Object> taskListExecutor = new TaskListExecutor<>(this.parallelQueries);

	for (int i = 0; i < this.parallelQueries; i++) {

	    for (queryIndex = 0; queryIndex < QUERIES_TYPES_LIST.size() - 1; queryIndex++) {

		taskListExecutor.addTask(new Runnable() {
			...
			try {
			    ...
			    Bond currentBond = createCountBond(queryIndex);

			    doCountQuery(currentBond, queryIndex);

			    doSearchQuery(currentBond, queryIndex);

			} catch (Exception ex) {

			    if (ex instanceof GSException) {

				GSLoggerFactory.getLogger(getClass()).error("Error occurred");
				handleGSException((GSException) ex);
			    } else {
	...
	taskListExecutor.executeAndWait(60);
    }

    /**
     * @param exception
     */
    void handleGSException(GSException exception) {
     ...
     * @throws GSException
     */
    private void doCountQuery(Bond currentBond, int queryTypeIndex) throws Exception {
     ...
     * @throws GSException
     */
    private void doSearchQuery(Bond currentBond, int queryTypeIndex) throws Exception {
	...
	// queriesInterval = 60000 / parallelQueries;
	...
	GSLoggerFactory.getLogger(getClass()).info("Queries per minute: " + parallelQueries);
	GSLoggerFactory.getLogger(getClass()).info("Index limit: " + indexLimit);
	GSLoggerFactory.getLogger(getClass()).info("Parallel queries: " + parallelQueries);
     ...
     * @return
     */
    private Bond createCountBond(int queryTypeIndex) {
	...
	builder.append("<td><li>Parallel queries: " + parallelQueries + "</li>");
	...
	String dirUri = "/stress-test-report/parallel/" + time + "/";
	...
	ParallelMarkLogicStressTest stressTest = new ParallelMarkLogicStressTest();

	args = new String[] { //

		"INDEX_LIMIT=1", //
		"TEST_LENGTH=600000", // 1000 * 60 * 10
		"PARALLEL_QUERIES=64", //
	...
	String parallelQueries = readArg(args, "PARALLEL_QUERIES");
	...
	stressTest.setParallelQueries(Integer.valueOf(parallelQueries));
	...
	// stressTest.setNote("10 parallel queries");
