...
//				createIndexWithGenericCLient(mapping);
	...
	public static OpenSearchClient createNoSSLContextClient(StorageInfo storageInfo) {
		...
		RestClientBuilder builder = RestClient.builder(httpHost)
				.setHttpClientConfigCallback(new RestClientBuilder.HttpClientConfigCallback() {
					...
					public HttpAsyncClientBuilder customizeHttpClient(HttpAsyncClientBuilder httpClientBuilder) {
						...
						try {
							sslContext = SSLContextBuilder.create().loadTrustMaterial(null, (chain, authType) -> true)
									.build();
		...
		if (storageInfo.getUser() != null && !storageInfo.getUser().isEmpty()) {
			if (storageInfo.getPassword() != null && !storageInfo.getPassword().isEmpty()) {
	...
	//
	@Override
	public List<String> getIdentifiers(IdentifierType type, String folderName, boolean excludDeleted)
			throws GSException {
	...
	private void createIndex(IndexMapping mapping) throws GSException {
		...
		try {
			...
			throw GSException.createException(getClass(), "OpenSearchDatabaseCreate" + mapping.getIndex() + "Error",
					ex);
		}
	}
